...
import { MOCK_USERS, MOCK_LOGS, MOCK_ROOMS } from '@/lib/mock-data';
import { 
  Table, 
  TableBody, 
  TableCell, 
...
export default function AdminDashboard() {
  ...
  // Derived stats
  const totalUsageToday = MOCK_LOGS.filter(log => 
    format(log.checkIn, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd')
  ).length;

  const activeSessions = MOCK_ROOMS.reduce((acc, room) => acc + room.activeSessions, 0);
  const mostActiveRoom = MOCK_ROOMS.sort((a, b) => b.activeSessions - a.activeSessions)[0];
      ...
      <main className="container mx-auto px-4 py-12 space-y-10">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          ...
          <StatCard 
            ...
            description="Professors in labs now"
            icon={DoorOpen}
          />
          <StatCard 
            title="Most Active Lab" 
            value={mostActiveRoom.name} 
            description="Highest traffic room"
            icon={DoorOpen}
          />
          <StatCard 
            title="Total Faculty" 
            value={MOCK_USERS.filter(u => u.role === 'professor').length} 
